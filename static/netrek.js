...
function addBot() {
    ...
    const teamCounts = { 1: 0, 2: 0, 4: 0, 8: 0 }; // Fed, Rom, Kli, Ori
    for (let i = 0; i < gameState.players.length; i++) {
        const player = gameState.players[i];
        if (player && player.status === 2) { // Alive players
    ...
    let minTeam = null;
    let minCount = 999;
    const teamNames = ['fed', 'rom', 'kli', 'ori'];
    const teamNums = [1, 2, 4, 8];
    
    for (let i = 0; i < teamNums.length; i++) {
        const teamNum = teamNums[i];
        if (teamCounts[teamNum] < minCount) {
            minCount = teamCounts[teamNum];
            minTeam = teamNames[i];
        }
    }
    
    // If player's team has least players, add bot to player's team
    // Otherwise add to the team with least players (for balance)
    let targetTeam = minTeam;
    const myTeamCount = teamCounts[myPlayer.team];
    
    // Check if player's team is outnumbered
    let enemyTotal = 0;
    for (let teamNum in teamCounts) {
        if (parseInt(teamNum) !== myPlayer.team) {
            enemyTotal += teamCounts[teamNum];
        }
    }
    
    // If player is outnumbered (enemies have 2+ more players total), help the player
    if (enemyTotal > myTeamCount + 1) {
        const myTeamName = teamNames[teamNums.indexOf(myPlayer.team)];
        targetTeam = myTeamName;
        // Player outnumbered, adding bot
    } else {
        // Otherwise, add to team with least players (prefer enemy teams)
        if (teamCounts[myPlayer.team] <= minCount) {
            // Player's team has least or tied for least, add to a random enemy team
            const enemyTeams = [];
            for (let i = 0; i < teamNums.length; i++) {
                if (teamNums[i] !== myPlayer.team) {
                    enemyTeams.push(teamNames[i]);
                }
            }
            targetTeam = enemyTeams[Math.floor(Math.random() * enemyTeams.length)];
        }
    }
    
    // Get a random ship type (0-3 for scout, destroyer, cruiser, battleship)
    const ship = Math.floor(Math.random() * 4);
    
    // Send chat command to add bot (no level parameter needed)
    const cmd = `/addbot ${targetTeam} ${ship}`;
    sendMessage({ 
        type: 'message', 
        data: { 
            text: cmd,
